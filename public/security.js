...
async function loadScanResults() {
  try {
    ...
    if (!data.results || data.results.length === 0) {
      container.innerHTML = '<div class="loading">尚未執行掃描。請點擊「立即掃描」按鈕。</div>';
      return;
    }
  ...
  try {
    ...
    resultDiv.innerHTML = `
      ...
      <div style="font-size: 12px; color: #666;">
        <strong>信心度：</strong> ${(result.confidence * 100).toFixed(0)}%
        ${result.tokensUsed ? ` | <strong>Token 消耗：</strong> ${result.tokensUsed}` : ''}
      </div>
    `;
...
document.getElementById('prompt-guard-toggle').addEventListener('change', async function() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      alert(`✅ Prompt Guard ${status}`);
    } else {
      alert('❌ 設定失敗');
      this.checked = !enabled; // 恢復原狀
    }
  } catch (error) {
